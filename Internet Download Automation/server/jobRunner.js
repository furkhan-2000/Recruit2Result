...
export function enqueueJob({
  ...
  const routeNote = proxyMode ? "NodeMaven proxy" : "local IP";
...
async function executeJob({
  ...
  log(
    ...
    "info",
    "Job started",
    `Headless browser for "${query}" · ${proxyMode ? "rotating NodeMaven proxy" : "local IP"} · ${parallelDownloads} parallel download(s)`
  );
  ...
  try {
    ...
    if (proxyMode) {
      if (!isProxyConfigured()) {
        throw new Error(
          "Proxy mode enabled but NodeMaven is not configured. Set NODEMAVEN_API_KEY in .env"
        );
      }
      const credentials = await resolveNodeMavenCredentials();
      proxyManager = createProxySessionManager({ credentials });
      log(id, "info", "NodeMaven credentials resolved", `Source: ${credentials.source}`);
    }
