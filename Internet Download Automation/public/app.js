...
function syncRouteUI() {
  ...
  if (proxyBtn) {
    proxyBtn.title = proxyConfigured
      ? "Route through NodeMaven — new IP per platform"
      : "Set NODEMAVEN_API_KEY in .env to enable";
  }
}
...
$$("#routeToggle .route-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    if (btn.dataset.mode === "proxy" && !proxyConfigured) {
      alert("NodeMaven not configured.\n\nSet NODEMAVEN_API_KEY in .env (Dashboard → Profile → API key).");
      return;
    }
...
function renderJob(job) {
  ...
  const routeLabel = job.proxy_mode ? "NodeMaven proxy" : "local IP";
