...
app.get("/api/extract/stream", async (req, res) => {
  ...
  if (proxyMode) {
    if (!isProxyConfigured()) {
      res.status(400).json({
        error: "Proxy mode enabled but NodeMaven not configured",
        hint: "Set NODEMAVEN_API_KEY in .env (Profile → API key in dashboard)",
    ...
    try {
      ...
      res.status(400).json({
        ...
        hint: "Use NODEMAVEN_API_KEY from Profile → API key, or proxy user/password from Proxy Setup",
        docs: getProxyPublicConfig().docs,
      });
      return;
    }
  ...
  try {
    pushLog(
      `Run started · ${sites.join(" + ")} · ${quantity} jobs/site · ${proxyMode ? "NodeMaven proxy" : "local IP"}`,
      "success"
    );
    ...
    for (const site of sites) {
      ...
      if (indeedBlockedOnProxy) {
        pushLog(
          "Indeed blocked NodeMaven proxy (HTTP 403) — retrying on local IP",
          "warn"
        );
    ...
    try {
      ...
      saveExtractionRun(runRecord);
      const intelligence = indexExtractionRun(runRecord);
      send("history-saved", {
        runId,
        path: `output/history/${runId}.json`,
        summary,
        intelligence: {
          totalJobs: intelligence.totalJobs,
