...
const DOCS = {
  ...
  proxySetup: "https://docs.nodemaven.com/en/articles/9596871-getting-started-with-residential-and-mobile-proxies",
  proxyString: "https://docs.nodemaven.com/en/articles/12663937-how-to-manipulate-a-proxy-string",
};
...
/** API may return boolean or "Yes"/"No" strings for is_traffic_frozen. */
...
/** True if API key or manual proxy user/password is set. */
export function isProxyConfigured() {
  return Boolean(getApiKey() || hasExplicitProxyCredentials());
}

/**
 * NodeMaven uses two auth layers (see API Swagger + proxy docs):
 * - API key → REST only (locations, stats, users/me) — NOT sent to gate.nodemaven.com
 * - proxy_username + proxy_password → Puppeteer / HTTP proxy tunnel
 * API key alone works when we fetch proxy credentials from GET /api/v2/base/users/me
 */
...
export function getProxyPublicConfig() {
  ...
  return {
    ...
    rotatePerSite: process.env.NODEMAVEN_ROTATE_PER_SITE !== "false",
    docs: DOCS,
    envHint:
...
export function buildNodeMavenUsername({
  baseUser,
  country,
  sessionId,
  filter = "medium",
}) {
  const base = baseUser.trim();
  let username = base;
  if (country && !base.includes("-country-")) {
...
export function createProxySessionManager({
  ...
  const country =
    countryOverride ||
    process.env.NODEMAVEN_PROXY_COUNTRY?.trim()?.toLowerCase() ||
    null;
  ...
  return {
    ...
    rotateForSite(site) {
      ...
      const entry = {
        ...
        country: country ?? "any",
        countryLabel: locationLabel ?? (country ? country.toUpperCase() : "Global"),
        host,
        port,
        protocol,
        filter,
